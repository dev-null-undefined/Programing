 ...
 * @author Martin
 */
public class FIleSorterByDataType {
    ...
    public static void checkIfSortedFolderIsDamaged(File f) {
        if (f.exists() && f.isDirectory()) {
            File check;
            for (String s : foldersInSortedFolder) {
                // console.append(s + "\n");
                check = new File(f.getAbsolutePath() + "\\" + s);
            ...
            console.append("DID YOU DELETE FOLDER: " + f.getName() + ". Making new one for you: " + f.mkdir() + "\n");
            checkIfSortedFolderIsDamaged(f);
        }
    }

    public static void redo(String notSortedFolder, String sortedFolder) {
        HashMap<String, File> hashMapOfFiles = new HashMap<>();
        for (File f : getFilesFromDir(new File(sortedFolder))) {
            hashMapOfFiles.put(f.getName(), f);

        }

        File notSortedFolderFile = new File(notSortedFolder);
        notSortedFolderFile.mkdir();
    ...
    public static void sort(String dir) { // Sorting folder
        ...
        File sortedFolder = new File(newDir);
        checkIfSortedFolderIsDamaged(sortedFolder);
